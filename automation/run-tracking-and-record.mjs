...
function buildBlocks({ label, report, testExitCode, startedAt, finishedAt }) {
  ...
  const blocks = [
    ...
    headingBlock('执行概览', 2),
    headingBlock('实验前风险与改良方案', 2),
    textBlock(report.executionPlan?.strategy ?? '未记录场景拆分策略。'),
    ...(report.executionPlan?.riskPoints ?? []).map((item) => textBlock(`风险：${item}`)),
    ...(report.executionPlan?.improvements ?? []).map((item) => textBlock(`改良：${item}`)),
