...
const inventoryDir = resolve('test-results/element-inventory');

async function saveInventory(page: Page, name: string, testInfo: TestInfo) {
  const data = JSON.stringify(await page.evaluate(inventoryScript), null, 2);
  mkdirSync(inventoryDir, { recursive: true });
  const outputPath = resolve(inventoryDir, name);
...
test.describe('核心生成链路 @core-smoke', () => {
  ...
  test('TC-03: 上传图片并生成后可加购进入 Checkout', async ({ page }, testInfo) => {
    ...
    await test.step('等待 3D 完成并横向拖动 2 秒', async () => {
      ...
      await saveInventory(page, 'generation-elements.json', testInfo);
    });
    ...
    await test.step('加入购物车并确认商品存在', async () => {
      ...
      await saveInventory(page, 'cart-elements.json', testInfo);
    });

    await test.step('进入 Checkout 并验证结账页', async () => {
      await cart.checkout();
      await new CheckoutPage(page).expectVisible();
      await saveInventory(page, 'checkout-elements.json', testInfo);
    });
  });
});
